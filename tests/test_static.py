...
assert metadata["version-name"] == "3.5.26"
assert metadata["version"] == 60
...
assert schema_keys["continuous-save-interval"].findtext("default") == "600"
assert schema_keys["continuous-save-interval"].find("range").attrib == {
    "min": "30",
    "max": "3600",
}
assert schema_keys["recall-enabled"].findtext("default") == "false"
assert schema_keys["experimental-wayland-session-management"].findtext("default") == "false"
...
assert 'version="3.5.26"' in build_script
...
assert "new Animation.Spinner(14, {animate: false, hideOnStop: true})" in source_text
assert "restoreActivity.begin()" in source_text
assert "restoreActivity.end()" in source_text
assert "this._savingIcon.play()" in source_text
assert "this._savingIcon.stop()" in source_text
...
assert 'CURRENT_VERSION = "3.5.26"' in app_source
...
assert "const windowCapture = await _captureWindowActors(" in source_text
assert "Math.max(60, energy.recallInterval)" in source_text
...
for support_module in (
    "recall_engine.py", "semantic.py", "restore_journal.py", "mcp.py", "capsule.py", "vault_key.py"
):
...
assert "automaticRestoreGroups(entries, completeQueue = false)" in source_text
assert "last-automatic-restore-attempt" in source_text
assert "automaticRestoreAttemptAllowed" in source_text
assert "_completeAutomaticRestore" in source_text
