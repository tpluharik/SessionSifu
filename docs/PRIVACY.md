...
| Portable window rules | Exact application identity, optional title substring, monitor, workspace and geometry | User action | Until individually deleted or SessionSifu data is removed |
...
| GNOME update cache | Downloaded Debian package | Created after update download | Until replaced or manually cleared |
| Portable update | Checksummed release archive at the user-selected destination | User action | Until the user deletes it |
