...
Version 3.5.23 combines the Ubuntu 26.04/GNOME Shell 50 integration with an
...
Version 3.5.23 coordinates launches, window layout and Recall screenshots through
...
GNOME browser restoration now leaves tabs and local documents to browser-owned
recovery, while retaining window placement and reusing starting instances. See
[Firefox and browser recovery](docs/RESTORE_GUIDE.md#firefox-and-browser-owned-recovery-on-gnome).
...
Download `sessionsifu_3.5.23_all.deb` from the matching GitHub Release, or build it
locally, then install it with:

```sh
sudo apt install ./sessionsifu_3.5.23_all.deb
```
...
```sh
sudo apt install ./dist/sessionsifu_3.5.23_all.deb
```
...
- `SessionSifu-3.5.23-windows-x64.zip`;
- `SessionSifu-3.5.23-macos-arm64.zip`;
- `SessionSifu-3.5.23-macos-x64.zip`; and
- `SessionSifu-3.5.23-linux-x64.tar.gz`.
...
On GNOME/Wayland, recovery processes every eligible application group in a
sequential queue, with eight seconds between groups. Starting applications have
up to 30 seconds to expose a running window; a timeout retains their records and
allows other applications to continue. Previous-desktop records are deduplicated
and bounded by the recorded window count (at most 32 per application).
An interrupted attempt briefly pauses automatic recovery for ten minutes and
holds the application that was in flight out of automatic recovery for 24 hours.
Other applications can recover at the next eligible login. Manual recovery can
retry held applications. The manager's **Restore progress** row reports progress,
paused attempts, failures and retained/skipped records.
...
On GNOME, version 3.5.23 saves application metadata across every workspace and
...
```text
dist/sessionsifu_3.5.23_all.deb
updates/latest.json
...
A pushed `v3.5.23` tag publishes the artifacts and `SHA256SUMS` as a GitHub
...
The current 3.5.23 release includes verified Czech and English fast Tesseract
