...
Download `sessionsifu_3.0.1_all.deb` from the matching GitHub Release, or build it
locally, then install it with:

```sh
sudo apt install ./sessionsifu_3.0.1_all.deb
```

When installing from this checkout, use:

```sh
sudo apt install ./dist/sessionsifu_3.0.1_all.deb
```
...
- `SessionSifu-3.0.1-windows-x64.zip`;
- `SessionSifu-3.0.1-macos-arm64.zip`;
- `SessionSifu-3.0.1-macos-x64.zip`; and
- `SessionSifu-3.0.1-linux-x64.tar.gz`.
...
```text
dist/sessionsifu_3.0.1_all.deb
updates/latest.json
...
A pushed `v3.0.1` tag publishes the artifacts and `SHA256SUMS` as a GitHub
...
The current 3.0.1 release establishes the encrypted visual Recall architecture while
