...
Version 2.2.0 retains the full Ubuntu 26.04/GNOME Shell 50 integration and adds
...
Download `sessionsifu_2.2.0_all.deb` from the `updates/` directory, or build it
locally, then install it with:

```sh
sudo apt install ./sessionsifu_2.2.0_all.deb
```

When installing from this checkout, use:

```sh
sudo apt install ./dist/sessionsifu_2.2.0_all.deb
```
...
- `SessionSifu-2.2.0-windows-x64.zip`;
- `SessionSifu-2.2.0-macos-arm64.zip`;
- `SessionSifu-2.2.0-macos-x64.zip`; and
- `SessionSifu-2.2.0-linux-x64.tar.gz`.
...
While Recall is active, `Ctrl+Alt+Space` opens a separate, compact search popup.
The shortcut can be disabled independently. GNOME uses its native Shell
...
```text
dist/sessionsifu_2.2.0_all.deb
updates/sessionsifu_2.2.0_all.deb
updates/latest.json
```
...
A pushed `v2.2.0` tag publishes the artifacts and `SHA256SUMS` as a GitHub
...
The current 2.2.0 release establishes the shared platform architecture while
