...
Version 3.2.1 combines the Ubuntu 26.04/GNOME Shell 50 integration with an
...
native pinch zoom and Ctrl+scroll zoom. The project is open source under
GPL-3.0.
...
Download `sessionsifu_3.2.1_all.deb` from the matching GitHub Release, or build it
locally, then install it with:

```sh
sudo apt install ./sessionsifu_3.2.1_all.deb
```

When installing from this checkout, use:

```sh
sudo apt install ./dist/sessionsifu_3.2.1_all.deb
```
...
- `SessionSifu-3.2.1-windows-x64.zip`;
- `SessionSifu-3.2.1-macos-arm64.zip`;
- `SessionSifu-3.2.1-macos-x64.zip`; and
- `SessionSifu-3.2.1-linux-x64.tar.gz`.
...
```text
dist/sessionsifu_3.2.1_all.deb
updates/latest.json
...
A pushed `v3.2.1` tag publishes the artifacts and `SHA256SUMS` as a GitHub
Release; ordinary pushes and pull requests build and retain test artifacts only.

## Roadmap

The modernized [SessionSifu roadmap](ROADMAP.md) separates the shipped 3.2.1
...
The current 3.2.1 release includes verified Czech and English fast Tesseract
