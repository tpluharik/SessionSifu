...
Download `sessionsifu_3.1.3_all.deb` from the matching GitHub Release, or build it
locally, then install it with:

```sh
sudo apt install ./sessionsifu_3.1.3_all.deb
```

When installing from this checkout, use:

```sh
sudo apt install ./dist/sessionsifu_3.1.3_all.deb
```
...
- `SessionSifu-3.1.3-windows-x64.zip`;
- `SessionSifu-3.1.3-macos-arm64.zip`;
- `SessionSifu-3.1.3-macos-x64.zip`; and
- `SessionSifu-3.1.3-linux-x64.tar.gz`.
...
```text
dist/sessionsifu_3.1.3_all.deb
updates/latest.json
...
A pushed `v3.1.3` tag publishes the artifacts and `SHA256SUMS` as a GitHub
...
The current 3.1.3 release fixes numeric Wayland window-ID matching so eligible
open windows receive their own encrypted Recall images. When Mutter cannot
render an inactive or minimized surface, SessionSifu shows an unavailable
preview instead of presenting pixels cropped from an unrelated foreground
window. Window-level OCR, bounded capture and exclusion rules remain intact.
